...
/* 
Status funcionou aqui, mas ainda não foi passado para teste.pl 21/06
...
confere_Visita(Posicao):-
    visitou(Posicao, X),
    (X = nao ->
        retract(visitou(Posicao, X)),
        assert(visitou(Posicao, sim));
        true
    ).
...
movimentacao(X):-
    ...
    descreva, confere_Visita(Vou_aqui),
    nl, !.
